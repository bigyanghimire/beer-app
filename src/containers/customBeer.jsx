  ...
  return (
    <>
      <ModalPopup show={show}>
        <Card
          cardClass="relative w-full max-w-2xl m-auto overflow-hidden"
          cardBorder="border-0"
          cardRadius="rounded"
          cardPadding="pt-6 pb-12 px-6"
        >
          <div className="pt-6 px-4">
            <h1 className="text-2xl font-bold">Add a New Beer</h1>
            <img
              className="mt-2 border-gray border rounded"
              src={houzzBeer}
              height="60"
              width="80"
            ></img>
            <form className="mt-4" onSubmit={handleSubmit(createNewCustomBeer)}>
              <div className="w-full lg:w-1/2 flex flex-col">
                <input
                  type="text"
                  className="border-gray border-2 bg-gray-100 p-2 rounded"
                  placeholder="Beer Name*"
                  id="beerName"
                  {...register("beerName")}
                />
                {errors.beerName && (
                  <small className="text-red-500">Beer Name is required</small>
                )}
                <input
                  type="text"
                  className="mt-4 border-gray border-2 bg-gray-100 p-2 rounded"
                  placeholder="Genre*"
                  id="genre"
                  {...register("genre")}
                />
                {errors.genre && (
                  <small className="text-red-500">Genre is required</small>
                )}
                <textarea
                  type="text"
                  className="mt-4 border-gray border-2 bg-gray-100 p-2 rounded"
